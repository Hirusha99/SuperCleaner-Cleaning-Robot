...
Ultrasonic sensor(TRIGGER_PIN, ECHO_PIN);  //Ultrasonic setup of pins
...
distance = sensor.read();                       //send ping, get distance in cm and store it in 'distance' variable:
...
while ((distance >1) && (distance < 15)) {
  ...
  if((Left_Value ==1) && (Right_Value==1)){            //check wheather the ultrasonic sensor's value stays between 1 to 15.
  //Move Forward:
  Motor1.setSpeed(speed);  //define motor1 speed:
  Motor1.run(FORWARD);   //rotate motor1 clockwise:
  Motor2.setSpeed(speed);  //define motor2 speed:
  Motor2.run(FORWARD);   //rotate motor2 clockwise:
  Motor3.setSpeed(speed);  //define motor3 speed:
  Motor3.run(FORWARD);   //rotate motor3 clockwise:
  Motor4.setSpeed(speed);  //define motor4 speed:
